	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '❌ Apenas adiministradores podem usar esse comando! ❌',
			...
			switch(command) {
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('O que você está procurando? em?')
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Promovido com sucesso @${mentioned[0].split('@')[0]} Agora você é um ADM 👔😎!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Rebaixado com sucesso @${mentioned[0].split('@')[0]} Você não é mais ADM👔😔!`, mentioned, true)
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país,+XX')
					try {
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você vai expulsar 🌝!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'listadmins':
					...
					break
                              case 'linkgroup':
                                      ...
                                      break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda a uma figurinha ❌')
					reply(mess.wait)
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Olá😳, bem vindo ao grupo, leia a descrição 🙃')
					if (Number(args[0]) === 1) {
